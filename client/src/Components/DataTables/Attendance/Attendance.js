...
const Attendance = () => {
  ...
  data.forEach((elem) => {
    table.push({
      Employee_ID: elem[0],
      Name: elem[1],
      Date: elem[2].split ? elem[2] : (new Date(Math.round((elem[2] - 25569)*86400*1000))).toISOString().substring(0, 10),
      in: elem[3],
      Out: elem[4],
      Duration: elem[5],
    });
  ...
  table.map((d) => {
    ...
    console.log("processDate",d.Date)
  ...
  const createRequests = async () => {
    ...
    await InTimes.map((i) => {
      ...
      console.log("date in excel import", i.Date)
      ...
      if (empData) {
        ...
        if (i.in.split(":")[1].length == 1) {
          formattedInTime = `${formattedInTime.split(":")[0]}:0${
            i.in.split(":")[1]
          }`;
        }
        ...
        if (i.out.split(":")[1].length == 1) {
          formattedOutTime = `${formattedOutTime.split(":")[0]}:0${
            i.out.split(":")[1]
          }`;
        }
